<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <body>
        <%@ include file="public/parts/header.jsp" %>
            ...
            <script>
                const rescheduleMedAppointment = document.getElementById('rescheduleMedAppointment');
                const closeIcon = document.querySelector('.close-icon')
                const modal = document.getElementById('modal-wrapper');
                closeIcon.onclick = () => modal.classList.remove('active-modal')
                rescheduleMedAppointment.onclick = () => {
                    ...
                    text.innerHTML = "Consulta reagendada para " + patientName + " no dia " + moment(date).format('L') + " às " + hour;
                }
                const getHourAndData = () => {
                    const modalContent = `Consulta agendada para ${patientName} no dia ${date} às ${hour}`;
